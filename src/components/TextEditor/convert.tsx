...
import { BlockType, CUSTOM_STYLE_MAP, EnitityType, InlineStyle } from "./config";
...
export const stateToHTML = convertToHTML<InlineStyle, BlockType>({
  ...
  entityToHTML: (entity, originalText) => {
    if (entity.type === EnitityType.link) {
      return (
...
export const HTMLtoState = convertFromHTML<DOMStringMap, BlockType>({
  ...
  htmlToEntity: (nodeName, node, createEntity) => {
    if(nodeName === 'a' && node.href) {
      return createEntity(EnitityType.link, 'MUTABLE', { url: node.href })
    }
